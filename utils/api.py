...
def get_educational_response(combined_text, user_query, subject="General", grade_level="High School", learning_style="Visual", lang_code="en"):
    """Get educational response from Groq API based on combined text and user query"""
    groq_api_key = st.session_state.get("groq_api_key", "")
    ...
    content_key = f"{combined_text}_{user_query}_{subject}_{grade_level}_{learning_style}"
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # Create prompt based on available content and query
            prompt = f"""
